...
/*
  ...
  ====================
  ver 3.4 : 14 Mar 2017
  - Added Alias, Enum, TypeDef Definitions and Handler classes
  ver 3.3 : 26 Feb 2017
...
{  
  ...
  ///////////////////////////////////////////////////////////////
  // rule to detect alias statements
  ...
  public:
	  bool doTest(const Scanner::ITokCollection* pTc) override
	  {
		  GrammarHelper::showParseDemo("Test alias definition: ", *pTc);
  ...
  ///////////////////////////////////////////////////////////////
  // action to add alias info to scope stack top

  class HandleAliasDefinition : public IAction
  {
	  Repository* p_Repos;
  public:
	  HandleAliasDefinition(Repository* pRepos)
	  {
		  p_Repos = pRepos;
	  }
	  ...
	  void doAction(const Scanner::ITokCollection* pTc) override
	  {
		  GrammarHelper::showParseDemo("Handle alias definition: ", *pTc);

		  p_Repos->currentAccess() = Access::publ;

		  size_t index1 = pTc->find("using") + 1;
		  size_t index2 = pTc->find("=");
		  std::string name;
		  for (size_t i = index1; i < index2; i++)
			  name += (*pTc)[i];

		  // create new ASTNode representing the alias
		  ASTNode* pElem = new ASTNode;
		  pElem->type_ = "alias";
		  pElem->name_ = name;
		  pElem->package_ = p_Repos->package();
		  ...
		  /*
		  * make this ASTNode child of ASTNode on stack top
		  * and there is NO need to push it on the stack
		  * because it is an alias that cannot have children
		  */
		  p_Repos->scopeStack().top()->children_.push_back(pElem);

		  GrammarHelper::showParse("alias def", *pTc);
	  }
  };
  ...
  public:
	  ...
	  void doAction(const Scanner::ITokCollection* pTc) override
	  {
		  ...
		  //size_t index1 = pTc->find("using") + 2;
		  size_t index1 = pTc->find("typedef") + 2;
		  ...
		  // create new ASTNode representing the type def
		  ASTNode* pElem = new ASTNode;
		  pElem->type_ = "type def";
		  pElem->name_ = name;
		  pElem->package_ = p_Repos->package();
		  pElem->startLineCount_ = p_Repos->lineCount();
		  pElem->endLineCount_ = p_Repos->lineCount();
		  ...
		  */
		  ...
		  GrammarHelper::showParse("type def", *pTc);
	  }
  };
